...
class BlockchainProcessor(Processor):
    ...
    def __init__(self, config, shared):
        ...
        self.okcashd_url = 'http://%s:%s@%s:%s/' % (
            config.get('okcashd', 'okcashd_user'),
            config.get('okcashd', 'okcashd_password'),
            config.get('okcashd', 'okcashd_host'),
            config.get('okcashd', 'okcashd_port'))
    ...
    def do_catch_up(self):
        ...
        self.header = self.block2header(self.okcashd('getblock', [self.storage.last_hash]))
    ...
    def okcashd(self, method, params=[]):
        postdata = dumps({"method": method, 'params': params, 'id': 'jsonrpc'})
        while True:
            try:
                respdata = urllib.urlopen(self.okcashd_url, postdata).read()
                break
            except:
    ...
    def get_header(self, height):
        block_hash = self.okcashd('getblockhash', [height])
        b = self.okcashd('getblock', [block_hash])
    ...
    def get_mempool_transaction(self, txid):
        try:
            raw_tx = self.okcashd('getrawtransaction', [txid, 0])
        except:
            return None
    ...
    def import_block(self, block, block_hash, block_height, sync, revert=False):
        ...
        # add undo info
        if not revert:
            self.storage.write_undo_info(block_height, self.okcashd_height, undo_info)
    ...
    def process(self, request, cache_only=False):
        ...
        if method == 'blockchain.numblocks.subscribe':
            ...
            try:
                txo = self.okcashd('sendrawtransaction', params)
            ...
            tx_hash = params[0]
            result = self.okcashd('getrawtransaction', [tx_hash, 0])

        elif method == 'blockchain.estimatefee':
            num = int(params[0])
            result = self.okcashd('estimatefee', [num])
    ...
    def getfullblock(self, block_hash):
        block = self.okcashd('getblock', [block_hash])
    ...
    def catch_up(self, sync=True):
        ...
        while not self.shared.stopped():
            ...
            # are we done yet?
            info = self.okcashd('getinfo')
            self.okcashd_height = info.get('blocks')
            okcashd_block_hash = self.okcashd('getblockhash', [self.okcashd_height])
            if self.storage.last_hash == okcashd_block_hash:
            ...
            self.up_to_date = False
            try:
                next_block_hash = self.okcashd('getblockhash', [self.storage.height + 1])
    ...
    def memorypool_update(self):
        mempool_hashes = set(self.okcashd('getrawmempool'))
